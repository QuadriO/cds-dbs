...
if (!cds.env.fiori.lean_draft) {
  throw new Error('"@quadrio/postgres" only works if cds.fiori.lean_draft is enabled. Please adapt your configuration.')
}

// requires @sap/cds-dk version >= 7.5.0
cds.build?.register?.(
  'postgres',
  class PostgresBuildPlugin extends cds.build.Plugin {
  ...
  async build() {
    ...
    async build () {
      ...
      if (fs.existsSync(path.join(this.task.src, 'package.json'))) {
        ...
        promises.push(
          this.write({
            dependencies: { '@sap/cds': '^7', '@quadrio/postgres': '^1' },
            scripts: { start: 'cds-deploy' },
          }).to('package.json'),
        )
      }
